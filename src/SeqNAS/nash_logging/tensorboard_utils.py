...
from .io_utils import is_file_empty
...
class TensorboardLogger:
    ...
    def __init__(
        ...
        self.tb_type = SummaryWriter
        logging.info("Setting up Tensorboard Logger...")
        self.base_tb_csv_dir = tb_csv_dir
        self.base_tb_dir = tb_dir
        self.tb_csv_dir = tb_csv_dir
        self.cur_run = None
        self._runs_stack = []
        self.flush_secs = flush_secs
        self.tb_writer = SummaryWriter(log_dir=tb_dir, flush_secs=flush_secs)
    ...
    def set_run(self, run_name=None):
        log_dir = self.base_tb_dir
        if run_name is not None:
            log_dir += "/" + run_name
        self.tb_writer.close()
        self.tb_writer = self.tb_type(log_dir=log_dir, flush_secs=self.flush_secs)

        self.tb_csv_dir = self.base_tb_csv_dir
        if run_name is not None:
            self.tb_csv_dir += "/" + run_name
        os.makedirs(self.tb_csv_dir, exist_ok=True)
    ...
    def log_scalars(self, tab="Train", scalars={}, phase_idx=0):
        name = "_".join(tab.split("/"))
        csv_name = f"{self.tb_csv_dir}/{name}_scalars.csv"
        csv = open(csv_name, "a")
        if is_file_empty(csv_name):
            csv.write(f"phase_idx,{','.join(scalars.keys())}\n")
    ...
    def log_metrics(self, tab="Train", metrics={}, phase_idx=0, rank=0):
        """
        ...
        # Log train/test accuracy
        metrics_string = f"{phase_idx},{rank}"
